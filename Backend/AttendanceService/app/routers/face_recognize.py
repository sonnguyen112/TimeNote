...
async def face_recognize(request: schemas.FaceRecognizeRequest, db: Session = Depends(get_db)):
    records = db.query(models.FaceEncoding).join(models.StudentManagement,models.StudentManagement.student_code==
                                                      models.FaceEncoding.student_id).filter(models.StudentManagement.course_id==request.course_id).all()
    face_encodings = list(map(lambda x:pickle.loads(bytes.fromhex(x.face_encoding)), records))
