...
for i in range(NUM_LECTURER):
    res = requests.post("http://localhost:8080/api/lecturer",json={
...
for i in range(NUM_LECTURER):
    res = requests.post("http://localhost:8080/api/course",json={
...
for stu in STUDENT_LIST:
    res = requests.post("http://localhost:8080/api/student", files={
...
res = requests.post("http://localhost:8080/api/course_detail",json={
    "courseCode" : "CS000",
    "classCode" : "20CTT",
    "timeStarts" : [
        "2 7:30",
